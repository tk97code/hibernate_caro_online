...
import allan.server.RunServer;
import allan.server.db.layers.BUS.GameMatchBUS;
import allan.server.db.layers.BUS.PlayerBUS;
import allan.server.db.layers.DTO.GameMatch;
import allan.server.db.layers.DTO.Player;
import allan.server.game.caro.Caro;
import allan.shared.constant.Code;
import allan.shared.constant.StreamData;
import allan.shared.helper.CustumDateTimeFormatter;
import allan.shared.helper.Line;
import allan.shared.security.AES;
...
//        String thisStatus = this.joinRoom(room, false);
//        String competitorStatus = cCompetitor.joinRoom(room, false);
//        sendData(StreamData.Type.JOIN_ROOM.name() + ";" + thisStatus);
...
//        sendData(StreamData.Type.DATA_ROOM.name() + ";success;" + data);
        ...
        this.cCompetitor = room.getClient1();
        this.cCompetitor.setcCompetitor(this);
        
        // send join room status to client
        sendData(StreamData.Type.JOIN_ROOM.name() + ";" + thisStatus); 
//        sendData(StreamData.Type.JOIN_ROOM.name() + ";" + competitorStatus);
//        cCompetitor.sendData();
       ...
       int id = cCompetitor.getLoginPlayer().getId();
       String email = cCompetitor.getLoginPlayer().getEmail();
       String avatar = cCompetitor.getLoginPlayer().getAvatar();
       String name = cCompetitor.getLoginPlayer().getName();
       String gender = cCompetitor.getLoginPlayer().getGender();
        
       cCompetitor.sendData(StreamData.Type.UPDATE_ROOM.name() + ";" + data);
    }
        ...
        switch (gameEventType) {
            case MOVE:
                ...
                if (!joinedRoom.isGameStarted()) {
                    ...
                    cCompetitor.sendData(StreamData.Type.SYNC_TIME.name());
                }
